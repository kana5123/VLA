"""Debug: model + env together to find the bottleneck."""
...
for step in range(5):
    ...
    # Model forward (with explicit CUDA sync)
    t_model = time.time()
    ...
    torch.cuda.synchronize()  # Force GPU completion before timing env
    ...
    print(f"  Step {step}: model={t_model_end-t_model:.2f}s, env={t_env_end-t_env:.3f}s, "
          f"total={t_env_end-t_total:.2f}s, action={action[:3].tolist()}", flush=True)
